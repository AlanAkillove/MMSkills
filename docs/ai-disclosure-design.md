...
| 论文声明 | 在参考文献之前二选一放置固定中文声明；未使用时也有固定句式 | 生成器必须按 2026 原文渲染；声明位置是参考文献**前**，不是之后 |
| 详情文件 | 使用 AI 时，支撑材料包含 PDF，文件名为“AI工具使用详情.pdf” | 文件名和提交位置由 profile 控制；生成前做规则检查 |
| 详情字段 | 工具名称/版本或型号；具体目的和环节；主要提示方式与过程（可附典型交互示例）；输出采纳、人工修改和核验主要情况（语言润色除外） | 事件 schema 支持完整/节选/摘要/未知；**不强制**全文关键交互记录 |
| 明确不要求 | 参考文献列出 AI 工具；正文标注 AI 生成内容 | 不得为“更合规”补 2025 字段 |
...
本适配器不能替代其他赛事规则。CUMCM 2026 官方正文、声明位置、详情字段和“不要求参考文献列工具/正文标注”已于 2026-09-01 对照官网与本地 PDF 写入 `references/rules/cumcm-2026.yaml`。赛区附加通知、提交系统和作者签核仍未完成前，profile 保持 draft，不能给出提交保证。
...
CUMCM 2026 AI 规定的官方正文、声明位置、详情字段和“不要求参考文献列工具/正文标注”已于 2026-09-01 对照官网与本地 PDF 冻结进 `references/rules/cumcm-2026.yaml`；赛区附加通知和提交系统仍需赛前再核。
