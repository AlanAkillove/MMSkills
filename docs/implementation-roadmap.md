...
1. `modeling-problem-intake`（已完成首轮生产实现与契约验证）；
2. `modeling-assumption-ledger`（已完成首轮生产实现与契约验证）；
3. `modeling-claim-evidence-audit`（已完成首轮生产实现与契约验证）；
4. `modeling-terminology-auditor`（已完成首轮生产实现与契约验证）；
5. `modeling-paper-reviewer`（已完成首轮生产实现与契约验证）；
6. `modeling-paper-naturalizer`（已完成首轮生产实现与契约验证）；
7. `modeling-final-preflight`（已完成首轮生产实现与契约验证）。

验收：一篇脱敏论文的数字、公式、引用、主张、术语和修订前后差异可以被复核；未知内容不会被补造。
...
阶段已完成。结果表明：论文和代码证据链具有较好的可复核基础，但题意决策账本、完整 AI 历史披露和最终冻结仍应保持人工门控；技能不能把局部材料误标记为“全流程完成”。

图表设计技能是在该轮回归之后新增的，尚未被计入上述真实项目回归结论；下一轮回归应重点检查图型选择、题目特异性、多面板取舍、中文字体和正文实际嵌入效果。
...
发布前仍需完成：全量 validators/tests 回归、相对链接和脚本可移植性核对、更多赛事 profile 的来源核验、Word/LaTeX 交付验证，以及对所有样例和日志字段的隐私复查。规则 profile 不应因为仓库公开而暴露用户私人会话或未公开竞赛材料。
