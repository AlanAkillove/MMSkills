...
普通单任务通常 0–2 个专项技能（预算，不是硬上限）。复合任务按能力覆盖选择最小集合。最终审稿必须独立 Subagent。
...
10. 官方规则与写作策略分开：后者在 `profiles/writing/`。CUMCM 中文终稿的 itemize、boxed 公式、孤立参考文献由预检机械执行。
11. 最终成稿审阅必须使用独立 Subagent，从磁盘重读稿件。
12. 契约测试通过不能宣称真实 Agent 行为已经验证。

明确不做：算法百科、固定候选/图数量、强制基线、把普通对话做成阶段流水线、按词频改写、把通用中文写作阈值硬套数模论文、把写作偏好写成官方规则。
