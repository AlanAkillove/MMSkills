...
> 当前状态：0.2.0 迭代已将编排器改为“用户意图优先、硬依赖与软建议分离”，增加专门的论文写作 skill 和成文质量机械检查。共享阶段注册表、finding/schema、运行档位和行为场景已经不仅存在于文档中：编排器会消费档位与 `user_intent` 生成 `effective_stage_policy`，入口按 schema 校验状态，finding 合并按 `canonical_issue_key` 去重。仓库 CI 仍使用正负 fixture；这不等于某个真实宿主模型已经通过行为测试。Codex runner 已接入，但 compatibility matrix 在没有真实宿主结果前保持 `fixture_only`。

## 0.2 的协作原则

用户当前目标优先于推荐阶段图。`depends_on` 是真正的硬依赖；`recommended_after` 只是建议先后。Agent 可以在核心内容尚未最终确认时给出候选、解释和暂定草稿，但不能把它们写成已采用的模型、结论或最终提交稿。
