...
> 当前状态：0.2.1 把 skills 进一步从流程控制器改成辅助知识。阶段依赖分成执行/采用/建议三层，确认挂在动作上，SKILL.md 保持薄入口，默认 TeX 不再预装论文目录。0.2.0 已引入意图优先编排、论文写作 skill 和成文清洁检查。仓库 CI 仍使用正负 fixture；这不等于某个真实宿主模型已经通过行为测试。Codex runner 已接入，但 compatibility matrix 在没有真实宿主结果前保持 `fixture_only`。
