...
| 先证据后成文 | 文献、代码、实验结果和术语先形成可核查依据；正式写整篇前先做图表论证计划，再进入正文 |
| 机械检查交给工具 | 引用、结果来源变化、图表事实、TeX/PDF 等确定性问题尽量交给工具；方法评价交给独立审阅 |
| 增量质量控制 | 承重产物先成为 candidate，审计查错、独立审阅评质；写完不等于完成 |
...
0.3.6 把质量策略变成可执行的状态转换约束，并让 Figure Designer 真正跨角色召回。正式写整篇时第一轮只做论证和图表计划。

0.3.5 把独立质量控制与图表前置写成运行约束：审计/审阅/挑战分层，承重单元先成为 candidate；写整篇前先考虑论证与图表计划；未规划图默认 diagnostic。CUMCM 中文写作策略禁止单独的「问题重述」章。

0.3.4 补上 Capability Index 的候选并入、多 facet 展开，以及合规门不再相信调用者布尔值。公共 writing profile 不再把 26–28 页当成默认目标。

0.3.2 把真实宿主回放里“规则写对了但 Agent 没执行”的问题下沉到提交上下文、写作 profile、机械预检和复合审稿路由。赛题年份不再自动变成提交规则年份；未解析 rules profile 不得声称合规；CUMCM 中文终稿的 itemize、boxed 公式和孤立参考文献由工具拦截。

0.3 系列已实现三角色入口、渐进加载、术语治理、结果快照、基础引用/图表/PDF 检查，以及证据约束的学术自然化。
