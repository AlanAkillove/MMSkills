...
先确认：赛事名称、**提交规则年份**（`rules_year`）、组别、赛区/学校、比赛时间窗、目标提交系统和需要检查的材料类型。赛题年份（`problem_year`）只描述题目来源，不能用来推断提交规则。2025 年赛题按 2026 年规则写时，必须显式选择 `cumcm-2026`。若这些信息不足，只生成缺口清单，不把赛题年份或默认赛事当成提交规则。
