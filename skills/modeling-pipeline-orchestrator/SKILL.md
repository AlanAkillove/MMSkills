...
默认不要用本技能处理普通对话。用户说“继续问题三”“比较几个模型”“重写摘要”时，走 `math-modeling` 三角色入口，不要生成 pipeline plan，不要读取 stage registry 或 process-freezer。

仅在这些情况使用编排器：完整赛程初始化、跨会话恢复、full audit、提交诊断、或用户明确要求下一步全图。新项目需要安排工作流、长会话需要判断下一步、或多个专项 skill 需要协调时也可以调用。如果用户已经明确本轮范围，这就是 user intent priority：优先推进该范围，不要为了补齐可选 artifact 而改写任务。
