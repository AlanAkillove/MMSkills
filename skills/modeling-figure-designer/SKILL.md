...
每张图还要有放置角色：`diagnostic`（调试，默认不进正文）、`comparison`、`paper`、`appendix`。代码运行中未规划的图默认 `diagnostic`，不能因为看起来不错就塞进论文。升级为 `paper` 必须补 visual brief、数据/单位 audit，以及独立 fresh-reader visual review。宁可少而承重，不要装饰清单。机械检查用 `math-modeling/scripts/figures/check_figure_placement.py`。
