...
逐张图/表回答：它服务哪个问题、来自什么数据、支撑哪条主张、读者应看什么、能否复核。检查数据/单位/图例/标签、正文解释和数值一致性。这是 **figure fact audit**，不是 fresh-reader visual review。后者由独立 Subagent 看渲染图+caption+拟支持的 claim，判断五秒内能否读懂、重点是否被遮挡、是否该用表。不把装饰图当证据，不改原始数据。
