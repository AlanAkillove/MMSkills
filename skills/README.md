...
专项 skill 是领域经验和检查方法，不是定义 Agent 如何思考的 SOP。常驻 `SKILL.md` 应保持薄：何时使用、少量硬约束、默认先直接帮用户解决问题；完整字段表、六步流程和账本格式放入 `references/`，按需读取。

除原始数据保护、来源真实性、隐私安全、陌生代码不自动执行和核心决定由人确认等硬边界外，读取顺序、检查深度、输出形式和步骤顺序都应根据用户目标、材料完整度、时间与风险取舍。默认先在对话中完成工作；只有跨会话、用户明确要求留痕、`working_depth=full` 或最终交付时才落盘各种 ledger。用户明确要求局部解释、探索、续写或修稿时，优先完成该范围，并把未覆盖内容写成范围说明，而不是无故阻断。
