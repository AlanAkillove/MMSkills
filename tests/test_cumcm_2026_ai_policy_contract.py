...
OBLIGATION = {"required", "prohibited", "not_required", "unspecified"}
...
def test_cumcm_2026_profile_drops_2025_bibliography_and_body_marks():
    ...
    assert disclosure["citation_of_ai_in_references"] == "not_required"
    assert disclosure["in_text_annotation_of_ai_content"] == "not_required"
    assert disclosure["unused_declaration_after_references"] == "not_required"
    assert disclosure["full_raw_log_required"] == "not_required"
    assert disclosure["typical_interaction_examples_allowed"] is True
    for field in (
        "citation_of_ai_in_references",
        "in_text_annotation_of_ai_content",
        "unused_declaration_after_references",
        "full_raw_log_required",
    ):
        assert disclosure[field] in OBLIGATION
        assert disclosure[field] is not False
        assert disclosure[field] is not True
...
def test_cumcm_2026_snapshot_does_not_treat_superseded_2025_clauses_as_required():
    ...
    assert "不得再执行的 2025 条款" not in text
